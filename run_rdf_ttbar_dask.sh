...
if [ -z "${merged}" ] ; then
    WDIR="rdf_ttbar_dask_orig_run_"
    datasets="ntuples.json"
else
    WDIR="rdf_ttbar_dask_merged_run_"
...
cat rdf_ttbar_dask_template.py | sed "s/_NFILES_/${nf}/" | sed "s/_AFNAME_/${afname}/" | sed "s/_WORKERS_/${workers}/" | sed "s/_DATASETS_/${datasets}/" | sed "s/_XCACHE_/${xcache}/" | sed "s/_MPOFF_/${mpoff}/" > ${WDIR}/rdf_ttbar_tmp.py
cd ${WDIR}
ln -s ../utils utils
ln -s ../helper.cpp .
ln -s ../ntuples.json .
